...
public class MG3_Key : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        {
            mouseTime = Time.time;
            vitricu = GameUIManager.Instance.cameraMinigameInFarm.ScreenToWorldPoint(Input.mousePosition);
        }
    }
    private void OnMouseUp()
    {
        if (!Util.IsMouseOverUI)
        {
            if (EventSystem.current.IsPointerOverGameObject() == false && Vector3.Distance(vitricu, GameUIManager.Instance.cameraMinigameInFarm.ScreenToWorldPoint(Input.mousePosition)) < 0.2f
    ...
    void HideKey()
    {
        ...
        {
            sequence.Append(spr[i].DOColor(new Color(1, 1, 1, 0), .2f));
        }
    }
